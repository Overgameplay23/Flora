...
import { fetchPet, resolvePetAvatarUri } from "../services/petService";
...
export default function PetChatScreen() {
  ...
  const [petRow, setPetRow] = useState<any>(null);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadPetState = async () => {
        ...
        try {
          const [state, pet] = await Promise.all([recomputePetState(), fetchPet(user.id)]);
          if (cancelled) return;
          setPetRow(pet || null);
          setPetName(typeof pet?.pet_name === "string" ? pet.pet_name.trim() : "");
          if (state) {
  ...
  const avatarUri = useMemo(
    () => resolvePetAvatarUri(petRow, profile?.pet_photo_url || null),
    [petRow, profile?.pet_photo_url]
  );

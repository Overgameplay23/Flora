...
import { fetchPet, toPetImageSources } from "../../src/services/petService";
...
export default function GardenScreen() {
  ...
  const loadPet = React.useCallback(async () => {
    ...
    try {
      ...
      const nextPetSources = toPetImageSources(pet, profile?.pet_photo_url || null);
      setPetSources(nextPetSources);
      const nextRenderSource = nextPetSources.stylized
        ? "stylized"
        : nextPetSources.cutout
          ? "cutout"
          : nextPetSources.photo
            ? "photo"
            : nextPetSources.original
              ? "originalMasked"
              : "placeholder";
      if (__DEV__) {
        console.log("GARDEN_RENDER_SOURCE_INIT", { renderSource: nextRenderSource });
      }

      const progress = await getGardenProgress(user.id);
      setProgressCount(progress.rows?.length ?? 0);
    } catch (error) {
      console.error("GARDEN_PET_LOAD_ERROR", { message: error?.message });
      setPetSources(toPetImageSources(null, profile?.pet_photo_url || null));
    }
  }, [profile?.pet_photo_url, user?.id]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <GardenScene petImageSources={petSources} />
      ...
      />
      <Pressable style={styles.progressButton} onPress={() => navigation.navigate("GardenProgress")}>
        <Text style={styles.progressButtonText}>Legacy Progress</Text>
      </Pressable>
    </SafeAreaView>
  );
}

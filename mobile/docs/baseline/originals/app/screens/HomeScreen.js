...
import { fetchPet, savePetName, toPetImageSources } from "../../src/services/petService";
...
import { completeTaskWithResilience, syncPendingCompletions, utcDateKey } from "../../src/services/taskCompletion";
...
function hydrateTasks(rawTasks) {
  ...
  return rawTasks.map((task, index) => {
    ...
    const icon = TASK_ICON_SET.includes(task.icon) ? task.icon : fallback.icon;
...
export default function HomeScreen() {
  ...
  const { user, profile, userStats } = useAuth();
  const homeDateKey = useMemo(() => todayKey(), []);
  const petPhoto = sanitizeLegacyPetUrl(profile?.pet_photo_url || null);
  ...
  const loadPetData = useCallback(async () => {
    if (!user?.id) {
      setPetSources((prev) => ({
        ...
        mask: null,
        photo: null,
        original: petPhoto || null,
        allowOriginal: !!petPhoto,
        alphaCutout: null,
        alphaStylized: null,
        maskReapplied: null,
        providerUsed: null,
    ...
    try {
      ...
      const nextPetName = normalizePetName(pet?.pet_name);
      const shouldPromptName = hasStylizedPetReady(pet) && !nextPetName;
      setPetName(nextPetName);
      setPetNameInput((prev) => (prev ? prev : nextPetName));
      setPetNameModalVisible(shouldPromptName);
      setPetSources(toPetImageSources(pet, petPhoto || null));
    } catch (error) {
      console.error("HOME_PET_LOAD_ERROR", { message: error?.message });
      setPetSources(toPetImageSources(null, petPhoto || null));
    }
  }, [petPhoto, user?.id]);
  ...
  useEffect(() => {
    const unsubscribe = subscribePetImageCache((cache) => {
      ...
      if ((cache.stylized || cache.cutout) && !normalizePetName(petName)) {
        setPetNameModalVisible(true);
      }
    });
    return unsubscribe;
  }, [petName, user?.id]);
    ...
    if (user?.id) {
      try {
        ...
        const mapped = rows.map((task, index) => ({
          ...
          icon: TASK_ICON_SET[index % TASK_ICON_SET.length],
          type: task.type,
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      loadTasks();
      loadRetentionData();
    }, [loadGardenData, loadPetData, loadTasks, loadRetentionData])
  );
  ...
  const streakText = profileStreak != null ? `Streak ${profileStreak}` : "Streak \u2014";
  const moodText = moodState ? `Pet mood: ${moodState}` : "Pet mood: \u2014";
  ...
  const handleToggleTask = useCallback(
    ...
    [completingTaskIds, loadGardenData, loadRetentionData, syncQueuedCompletions, tasks, user?.id]
  );
  ...
  const retentionStreakDays = Number.isFinite(Number(petRetentionState?.streak_days))
    ...
    : 0;
  const retentionRiskScore = Number.isFinite(Number(petRetentionState?.risk_score))
    ? Math.max(0, Math.floor(Number(petRetentionState?.risk_score)))
    : 0;
  ...
  }, [loadPetData, petNameInput, user?.id]);
    ...
    <LinearGradient colors={["#0f1420", "#121826"]} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.petNameCard}>
          ...
          <Text style={styles.petNameValue}>{petDisplayName}</Text>
        </View>
        ...
        <View style={styles.retentionCard}>
          <Text style={styles.retentionTitle}>Pet Retention</Text>
          <Text style={styles.retentionMeta}>
            Streak {retentionStreakDays}d · Mood {retentionMoodLabel} · Risk {retentionRiskScore}
          </Text>
          {loadingRetention ? (
            <ActivityIndicator color="#35d07f" style={styles.retentionLoading} />
          ) : retentionMetrics.length === 0 ? (
            <Text style={styles.retentionRow}>No activity data yet.</Text>
          ) : (
            retentionMetrics.map((row) => (
              <Text key={row.day} style={styles.retentionRow}>
                {toWeekdayFromDateKey(row.day)}: {row.activity_count} activities, {row.points_earned} pts
              </Text>
            ))
          )}
        </View>
